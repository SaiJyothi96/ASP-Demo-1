...
import pymysql
import os
conn = pymysql.connect(host="localhost", user="root", password="mahesh@123", db="modern_agriculture")
cursor = conn.cursor()
app = Flask(__name__)
...
agriculture_System_Access_Key = "AKIARTLO5HQWA5N5EA74"
agriculture_System_Secret_Access_Key = "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"
agriculture_System_bucket = "agriculture-s3-bucket-project"
agriculture_System_s3_client = boto3.client('s3', aws_access_key_id=agriculture_System_Access_Key, aws_secret_access_key=agriculture_System_Secret_Access_Key)
...
def admin_home():
    from_datetime_str = request.args.get("from_date")
    to_datetime_str = request.args.get("to_date")
    machinery_count = 0
    machinery_booking_count = 0

    cursor.execute("SELECT COUNT(*) FROM machineries")
    machinery_count = cursor.fetchone()[0]

    if from_datetime_str and to_datetime_str:
        try:
            from_datetime = datetime.datetime.strptime(from_datetime_str, "%Y-%m-%dT%H:%M")
            to_datetime = datetime.datetime.strptime(to_datetime_str, "%Y-%m-%dT%H:%M")

            query = """
                SELECT COUNT(*) FROM bookings
                WHERE from_date_time <= %s AND to_date_time >= %s
            """
            cursor.execute(query, (to_datetime, from_datetime))
            machinery_booking_count = cursor.fetchone()[0]
        except ValueError:
            print("Invalid datetime format. Use YYYY-MM-DDTHH:MM")

    return render_template("admin_home.html",
                           machinery_count=machinery_count,
                           machinery_booking_count=machinery_booking_count)
...
def add_machinery1_action():
    ...
    picture.save(path)
    agriculture_System_s3_client.upload_file(path, agriculture_System_bucket, picture.filename)
